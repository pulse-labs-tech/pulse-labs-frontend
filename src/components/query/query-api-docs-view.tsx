...
interface QueryApiDocsViewProps {
  locale: string;
}

export function QueryApiDocsView({ locale }: QueryApiDocsViewProps) {
  ...
  return (
    <div className="min-h-screen bg-auth-bg text-auth-text relative overflow-x-hidden flex flex-col">
      ...
      {/* Main App Header */}
      <AppHeader active="query" locale={locale} />
      ...
      <main className="container-focused flex-grow py-8 relative z-10 flex flex-col">
        ...
        <div>
          ...
          >
            ...
            <span>{locale === "vi" ? "Quay lại Hỏi đáp AI" : "Back to Ask AI"}</span>
          </Link>
        </div>
