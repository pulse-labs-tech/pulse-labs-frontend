...
export function Header() {
  ...
  return (
    <header className="fixed top-0 right-0 left-0 z-50 border-b border-auth-border-subtle bg-auth-bg/70 backdrop-blur-2xl backdrop-saturate-150">
      <nav className="mx-auto flex h-14 max-w-7xl items-center justify-between px-5 lg:px-8 3xl:h-16 3xl:max-w-[1680px] 4xl:max-w-[2200px]">
        ...
        <button type="button" onClick={() => setMobileOpen(!mobileOpen)} className="flex h-9 w-9 items-center justify-center rounded-lg border border-auth-border-subtle bg-white/5 text-auth-text-2 transition-all duration-200 hover:bg-white/10 hover:text-white active:scale-95 md:hidden" aria-label={mobileOpen ? "Đóng menu" : "Mở menu"} aria-expanded={mobileOpen}>
          {mobileOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
        </button>
      </nav>
      {mobileOpen && (
        <div className="border-t border-auth-border-subtle bg-auth-bg/95 px-5 pb-6 pt-4 backdrop-blur-2xl md:hidden">
          <div className="flex flex-col gap-4">
            ...
            <hr className="border-auth-border-subtle" />
