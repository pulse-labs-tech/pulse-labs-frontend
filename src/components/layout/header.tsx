...
export function Header() {
  ...
  return (
    <header className="fixed top-0 right-0 left-0 z-50 border-b border-white/[0.08] glass-premium">
      <nav className="w-full max-w-[1440px] mx-auto px-6 lg:px-12 flex h-14 lg:h-16 items-center justify-between relative transition-all duration-300 ease-in-out">
        <div className="flex justify-start z-10">
          <Link href={`/${locale}`} className="group flex items-center gap-2.5" aria-label="Pulse Knowledge — trang chủ">
            ...
            <span className="text-sm font-extrabold tracking-tight text-auth-text lg:text-base transition-colors">
              Pulse<span className="bg-gradient-to-r from-brand-400 to-accent-300 bg-clip-text text-transparent">Knowledge</span>
            </span>
          </Link>
        </div>
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden justify-center items-center gap-10 lg:gap-12 lg:flex">
          {navLinks.map((l) => (
        ...
        <div className="hidden justify-end items-center gap-6 lg:gap-8 lg:flex z-10">
        ...
        <div className="flex items-center gap-3 justify-end lg:hidden z-10">
