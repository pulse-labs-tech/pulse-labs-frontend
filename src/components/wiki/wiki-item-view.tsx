...
function getSourceTypeLabel(type: WikiSourceType): string {
  switch (type) {
    ...
    case "manual_note": return "Ghi chú thủ công";
    ...
    case "indexed":
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-950/40 border border-emerald-500/20 text-emerald-400">
          <CheckCircle2 className="h-3.5 w-3.5" /> Đã lập chỉ mục
        </span>
      );
    case "pending":
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-blue-950/40 border border-blue-500/20 text-blue-400 animate-pulse">
          <Clock className="h-3.5 w-3.5" /> Đang xử lý
        </span>
      );
    case "degraded":
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-950/40 border border-amber-500/20 text-amber-400">
          <AlertTriangle className="h-3.5 w-3.5" /> Chất lượng thấp
        </span>
      );
    case "failed":
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-red-950/40 border border-red-500/20 text-red-400">
          <XCircle className="h-3.5 w-3.5" /> Lỗi chỉ mục
        </span>
      );
    default:
      return (
...
export function WikiItemView({ id }: WikiItemViewProps) {
    ...
    try {
      ...
      if (res.status === "1" && res.data?.item) {
        ...
        if (res.error_code === "NOT_FOUND" || res.error_code === "WIKI_ITEM_NOT_FOUND") {
          ...
          setErrorMsg(res.msg || "Không thể tải thông tin wiki item này.");
        }
      }
    } catch {
      setErrorMsg("Không kết nối được máy chủ. Vui lòng kiểm tra lại kết nối mạng.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    const t = setTimeout(() => fetchItem(), 0);
  ...
  if (isNotFound) {
    return renderLayout(
      <div className="flex flex-col items-center justify-center py-20 gap-6 text-center">
        ...
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-red-400 mb-2">404 — Không tìm thấy</p>
          <h2 className="text-xl font-extrabold text-auth-text">Wiki Item không tồn tại</h2>
          <p className="text-xs text-auth-text-2 mt-2 max-w-xs leading-relaxed">
            Mục wiki này có thể đã bị xóa, di chuyển, hoặc ID không hợp lệ.
          </p>
        </div>
        <Link
          href="/wiki"
  ...
  if (errorMsg && !item) {
    return renderLayout(
      <div className="flex flex-col items-center justify-center py-20 gap-6 text-center">
        ...
        <div>
          <h2 className="text-xl font-extrabold text-auth-text">Không thể tải dữ liệu</h2>
    ...
    <div className="flex flex-col gap-6 lg:flex-row lg:gap-8 lg:items-start">
      ...
      <div className="flex-grow flex flex-col gap-6 min-w-0">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-auth-text-3 flex-wrap" aria-label="Breadcrumb">
          <Link href="/dashboard" className="hover:text-auth-text transition-colors">Dashboard</Link>
          <ChevronRight className="h-3 w-3 shrink-0" />
          <Link href="/wiki" className="hover:text-auth-text transition-colors">Wiki</Link>
          {item.domain?.name && (
            <>
              <ChevronRight className="h-3 w-3 shrink-0" />
              <Link
                href={`/wiki?domainId=${item.domain.id}`}
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            <ArrowLeft className="h-3 w-3" /> Thư viện Wiki
          </Link>
        ...
        <div className="bg-auth-surface/40 border border-white/[0.06] backdrop-blur-md rounded-2xl p-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-bold tracking-tight uppercase text-auth-text-3">Tóm tắt</h2>
          </div>
          <p className="text-sm text-auth-text-2 leading-relaxed whitespace-pre-wrap">
            {item.summary || "Không có tóm tắt cho mục này."}
          </p>
        </div>
        ...
        {item.concepts && item.concepts.length > 0 && (
          <div className="bg-auth-surface/40 border border-white/[0.06] backdrop-blur-md rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-bold tracking-tight uppercase text-auth-text-3">Concepts liên quan</h2>
            </div>
        ...
        {item.citations && item.citations.length > 0 && (
          <div className="bg-auth-surface/40 border border-white/[0.06] backdrop-blur-md rounded-2xl p-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-bold tracking-tight uppercase text-auth-text-3">Phân đoạn tài liệu (Chunks)</h2>
            </div>
        ...
        {(item.source?.url || item.source?.titleHint) && (
          <div className="flex items-center gap-3 bg-auth-elevated/60 border border-auth-border rounded-xl px-4 py-3">
            ...
            <span className="text-xs text-auth-text-3">
              ...
              <span className="font-medium text-auth-text-2">{getSourceTypeLabel(item.source.sourceType)}</span>
              {" — "}
              {item.source.url ? (
                <a
      ...
      <aside className="w-full lg:w-[280px] shrink-0 flex flex-col gap-4 lg:sticky lg:top-24">
        ...
        <div className="bg-auth-surface/40 border border-white/[0.06] backdrop-blur-md rounded-2xl p-5">
          <p className="text-[10px] font-bold uppercase tracking-wider text-auth-text-3 mb-3">Thống kê</p>
          <div className="flex flex-col gap-3">
            {[
              {
                icon: <AlignLeft className="h-3.5 w-3.5" />,
                label: "Số từ",
                value: formatNumber(item.source?.wordCount),
              },
              {
                icon: <Layers className="h-3.5 w-3.5" />,
                label: "Số chunks",
                value: formatNumber(item.citations?.length),
              },
              {
                icon: <Calendar className="h-3.5 w-3.5" />,
                label: "Biên soạn lúc",
                value: formatDateVN(item.compiledAt),
              },
              {
                icon: <RefreshCw className="h-3.5 w-3.5" />,
                label: "Cập nhật lúc",
                value: formatDateVN(item.updatedAt),
              },
        ...
        <Link
          href={`/query?roleKbId=${item.roleKbId}&q=${encodeURIComponent(item.title)}`}
        ...
        {item.relatedItems && item.relatedItems.length > 0 && (
          <div className="bg-auth-surface/40 border border-white/[0.06] backdrop-blur-md rounded-2xl p-5">
            <p className="text-[10px] font-bold uppercase tracking-wider text-auth-text-3 mb-3">Mục liên quan</p>
            <div className="flex flex-col gap-2">
              {item.relatedItems.slice(0, 3).map((related) => (
                <Link
                  key={related.id}
                  href={`/wiki/items/${related.id}`}
